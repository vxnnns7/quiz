...
localStorage.setItem('cLangquestions', JSON.stringify(cLangObj))
displayData()
function displayData() {
    let getcLangquestions = JSON.parse(localStorage.getItem('cLangquestions'))
    console.log(getcLangquestions);
    str = ''
    getcLangquestions.map((v) => {
...
function submitQuiz() {
    ...
    const userAnswersArray = cLangobj.map(question => {
    ...
    const score = userAnswersArray.reduce((totalScore, userAnswer, index) => {
        const correctAnswer = cLangobj[index].correctanswer;
